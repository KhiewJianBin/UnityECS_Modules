...
public partial struct EmptyBuffSystem : ISystem, ISystemStartStop
{
    [BurstCompile]
    public void OnCreate(ref SystemState state)
    {
        EntityQuery query = state.GetEntityQuery(ComponentType.ReadWrite<EmptyBuffComponent>());
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (buffcomp, entity) in SystemAPI.Query<RefRW<EmptyBuffComponent>>().WithEntityAccess())
        {
            // Apply Buff
            ...
            if(Expired)
            {
                ecb.RemoveComponent<EmptyBuffComponent>(entity);
